...
/*
int singleMessagePriorityTestTetrinet() {
  pickedWorkers = 0;
  signal(SIGCHLD, SIG_IGN);
  signal(SIGSTD, SIG_IGN);
  printf("MANAGER DBG: SIGRTMIN+4 is %lu, SIGSTD is %lu \n", SIGRTMIN+4, SIGSTD);
  nextMsgToVerify = single_msg_to_verify;
  nextC2SMsg = single_msg_to_verify;
  fflush(stdout);
  //tase_exit();
  setTetrinetBaseFeats();

  //printf("Calling loadPredModel\n");fflush(stdout);
  //loadPredModel();
  //printf("Calling init_structuresPQ \n");fflush(stdout);
  //init_structuresPQ(&Stopped, &Running);
  printf("Calling initial_forkPQ \n");fflush(stdout);
  int firstPid = initial_forkPQ(Stopped, Running);
  //printf("Calling manage_workersPQ from pid %d \n", getpid());fflush(stdout);
  if (firstPid == 0) {
    ...
    signal(SIGSTD, SIG_IGN);
    printf("Calling manage_workersPQ from pid %d \n", getpid());fflush(stdout);
    singleMessagePriorityTestStart = util::getWallTime();
    manage_workersPQ(Stopped,Running);
  }
  return firstPid;
}
*/
