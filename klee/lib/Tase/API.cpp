...
void send_sigchld(int signal, siginfo_t *info, void *context) {
  *tase_error = true;
  kill(manager, SIGCHLD);
  exit(1);
}
...
pid_t initial_fork(struct WorkerGroup * Stopped, struct WorkerGroup * Running) {
  ...
  if( child != 0 ) {
    ...
    *explorer_get_next_pid = 1;
    ...
    sigact.sa_sigaction = &send_sigchld;
...
// don't print here, not safe in signal handler. see https://man7.org/linux/man-pages/man7/signal-safety.7.html
// looks like the semaphore calls are also unsafe here... rework like the Scout?
...
void init_structures(struct WorkerGroup ** Stopped, struct WorkerGroup ** Running) {
  ...
  ms_size = sizeof(struct WorkerGroup) * 2 + sizeof(int*)*2 + sizeof(pid_t) * MAX_GROUP_SIZE * 2 + 2*sizeof(bool);
  ...
  *Running = ((struct WorkerGroup*) ms_base)+1;
  ...
  tase_error = (bool*)(success + sizeof(bool));
...
void worker_error(struct WorkerGroup *Stopped, struct WorkerGroup *Running) {
  get_sem_lock();
  *tase_error = true;
  release_sem_lock();
