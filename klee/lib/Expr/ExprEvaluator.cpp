...
ExprVisitor::Action ExprEvaluator::evalRead(const UpdateList &ul,
                                            unsigned index) {
  if (inModelPCS) {
    LOG_TASE("Entered ExprEvaluator::evalRead \n");LOG_FLUSH();
  }

  for (const UpdateNode *un=ul.head; un; un=un->next) {
    if (inModelPCS) {
      LOG_TASE("ExprEvaluator::evalRead about to call visit \n");LOG_FLUSH();
    }
    ref<Expr> ui = visit(un->index);
    if (inModelPCS) {
      LOG_TASE("ExprEvaluator::evalRead returned from visit \n");LOG_FLUSH();
    }
...
ExprVisitor::Action ExprEvaluator::visitRead(const ReadExpr &re) {
  if (inModelPCS) {
    LOG_TASE("In ExprEvaluator visitRead \n");LOG_FLUSH();
  }

  ref<Expr> v = visit(re.index);
  if (inModelPCS) {
    LOG_TASE("ExprEvaluator visitRead returned from read \n");LOG_FLUSH();
  }

  
  if (ConstantExpr *CE = dyn_cast<ConstantExpr>(v)) {
    if (inModelPCS) {
      LOG_TASE("ExprEvaluator about to call evalRead \n");LOG_FLUSH();
    }
    ExprVisitor::Action res = evalRead(re.updates, CE->getZExtValue());
    if (inModelPCS) {
      LOG_TASE("ExprEvaluator called evalRead \n");LOG_FLUSH();
    }
    return res;
      //return evalRead(re.updates, CE->getZExtValue());
  } else {
    if (inModelPCS) {
      LOG_TASE("ExprEvaluator in doChildren case \n");LOG_FLUSH();
    }
