...
extern UFManager *ufmanager_cheat; //Should hold all info in ufmanager plus "cheat" constraints
//representing client inputs for use in training a path selection oracle.
...
void updateUFStructures(std::vector<const klee::Array *> arrs, ref <Expr> e, bool cheat) {
  ...
  if (cheat) {
    ...
    if (arrs.size() == 1 ) {
      int ufe_Cheat = arrs.front()->UFE_Cheat;
      int rep = ufmanager_cheat->find(ufe_Cheat);
      (*ufmanager_cheat)[rep].constraints.push_back(e);
      return;
    }
    ...
    while (it2 != arrs.end()) {
      int rep = ufmanager_cheat->join((*it1)->UFE_Cheat, (*it2)->UFE_Cheat);
...
void ConstraintManager::addConstraint(ref<Expr> e, std::vector<const klee::Array * > * symNames) {
  static double addCTime = 0.0;
  double T0 = util::getWallTime();
  //LOG_TASE("Calling addConstraint for expr: %s \n", e->dumpToStr().c_str());LOG_FLUSH();
  if (symNames != NULL) {
    printf("addConstraintInternal case 1 \n");fflush(stdout);
    addConstraintInternal(e, symNames);
  } else {
    printf("addConstraintInternal case 2 \n");fflush(stdout);
    addConstraintInternal(e);
    printf("Returned from addConstraintInternal case 2 \n");fflush(stdout);
  }

  double diff = util::getWallTime() - T0;
  addCTime += diff;
  printf("%f second spent calling addConstraint; %f seconds total so far \n", diff, addCTime);
  fflush(stdout);
}
  
				      
//ABH Added option to pass in list of symbolic variables and avoid calling "findSymbolicObjects"
void ConstraintManager::addConstraintInternal(ref<Expr> e,  std::vector<const klee::Array * > * symNames, bool cheat) {
  ...
  case Expr::And: {
    ...
    //Not passing symNames here might be a bug, but it's how I have it in my build.
    //We should test it after the merge.  -ABH 01/29/2024
    addConstraintInternal(be->left, NULL, cheat);
    addConstraintInternal(be->right, NULL, cheat);
    break;
  }
  ...
  case Expr::Eq: {
    ...
    */
    if (useUF) {
      if ( symNames != NULL ) {
	updateUFStructures(*symNames, e, cheat);
      } else {
	std::vector<const klee::Array * > arrays;
	double T0 = util::getWallTime();
	klee::findSymbolicObjects(e, arrays);
	printf("Spent %lf seconds on findSymbolicObjects \n", util::getWallTime() - T0);fflush(stdout);
	//printf("Found %d arrays when adding constraint \n", arrays.size());
	T0 = util::getWallTime();
	updateUFStructures(arrays, e, cheat);
	printf("Spent %lf seconds on updateUFStructures \n", util::getWallTime() - T0);fflush(stdout);
      }
    }
    if (cheat)
      cheatConstraints.push_back(e);
    else
...
void ConstraintManager::addCheatConstraint(ref<Expr> e) {
  printf("Adding cheat constraint \n");
  fflush(stdout);
 //DANGER - make sure this triggers
  //the logic downstream to appropriately find dependent constraints based on expr e.
  addConstraintInternal(e, NULL, true);
}
