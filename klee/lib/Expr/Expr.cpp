...
bool useCheatCons = true; // Might be OK to just remove this variable.  I don't think it
//triggers any special logic unless we are analyzing a project with system call models that
//explicitly tries to add cheat constraints.
...
void Expr::dumpToStdout() {
  this->print(outs());
  outs() << "\n";
  outs().flush();
}

std::string Expr::dumpToStr() {
  std::string outStr;
...
Array::Array(const std::string &_name, uint64_t _size,
  ...
  if (useCheatCons) {
    UFE_Cheat = ufmanager_cheat->create(this);
  }
