...
private:
  ...
  //void model_inst();
  void impliedValueConcRegs();
  void calleeWipePastStack();
  void calleeKillDeadRegs();
  void killDeadRegsPreCall();
  void killDeadRegs( uint64_t prev_rip );
  void do_ret();
  ...
  void fastWipeReg(void * ptr, std::vector<uint8_t> bytes);
  ...
  //MAVLINK models
  void model_mavlink_write_tase();
  void model_peek_channel_state();
  void forkOnPossibleRIPValuesMavlink(ref <Expr> inputExpr, uint64_t initRIP);
  int forkOnPossibleIRBranchesMavlink(ref<Expr>, std::vector<std::vector<float>> IR_Constraint_BOWs);
  void mavlinkTrainingFork(ref <Expr> inputExpr, uint64_t initRIP);
  int mavlinkTrainingIRFork(ref<Expr> condition);
  void dumpFeaturesMavlink(bool isValidState,  int LogID, uint64_t BBID );
  
  //TetriNET models 
  void dumpFeaturesTetriNET(bool isValidState,  int ID, uint64_t MLBBID);
  void tetrinet_make_symbolic(uint64_t addr, uint64_t size, char * name);
  void model_tetri_ktest_write();
  void model_tetri_sgets();
  void model_tetri_conn();
  void model_srand();
  void model_tetri_vsnprintf();
  void model_tetri_shutdown();
  void forkOnPossibleRIPValuesTetriNET(ref<Expr>, uint64_t);
  int forkOnPossibleIRBranchesTetriNET(ref<Expr>, std::vector<std::vector<float>> IR_Constraint_BOWs);
  void tetrinetTrainingFork(ref <Expr> inputExpr, uint64_t initRIP);
  void model_tetri_atoi();

  int getCurrentBranchDest(bool isTrueBranch);
  int forkOnPossibleIRBranches( ref<Expr> condition, std::vector<std::vector<float>> IR_Constraint_BOWs);
  int tetrinetTrainingIRFork(ref<Expr> condition);
  //Training functionss
  void dumpFeatures(bool isValidState,  int ID);
  void tetrineTrainingFork(ref <Expr> inputExpr, uint64_t initRIP);
  ref<Expr> toUniquePreCalc(ref <Expr> &e, bool cheat);
  std::vector<ref<Expr>> getDepCons(ref <Expr> &e, bool cheat);
  void addCheatConstraint(ref <Expr> e);
  void printNPossibleValues(ExecutionState &s, ref<Expr> e, int n );
  unsigned getExprConstraintHashKey( std::vector<ref<Expr>> constraints, ref <Expr> e);
  void loadGetNPossibleValuesCache();
  void printGetNPossibleValuesCache();
  void updateGetNPossibleValuesCache(std::vector<ref<Expr>> constraints, ref <Expr> e, std::vector<ref<Expr>> vals);
  std::vector<ref<Expr>> checkGetNPossibleValuesCache(std::vector<ref<Expr>> constraints, ref<Expr> e);
  std::vector<ref<Expr>> getNPossibleValues(ExecutionState &s, ref<Expr> e, int n, bool failOnTooBig = true );
  void addConstraintBOW(ref<Expr> expr, std::vector<double>& features);
  void remConstraintBOW(ref<Expr> expr, std::vector<double>& features);
  void addConstraintFeature(ref <Expr> e);
  void remConstraintFeature(ref <Expr> e);
  std::vector<std::vector<float>> getRIPConstraintBOWs(ref<Expr> inputExpr,  std::vector<ref<Expr>> destExprs);
  //Symbolic offsets
  ref<Expr> handleSymbolicOffset(ref<Expr> offset, const ObjectState * os, Expr::Width type);
...
public:
  ...
  virtual void processSyntheticPath(int msgIdx);
