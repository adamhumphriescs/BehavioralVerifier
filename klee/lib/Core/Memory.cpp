...
ObjectState::~ObjectState() {
  ...
  //Removed the call to delete for TASE because
  //we're backing persistent parts of the address space (e.g., globals
  //and the stack) with object states.  This is probably causing memory
  //leaks for some temporary object state allocations, so we should
  //possibly add a "forTASE" arg to disambiguate those cases.
...
/***/
//if ( offset< rsp_global || isByteConcrete(offset)) { //ABH: This was causing
//symbolic bytes to be incorrectly identified as concrete in interp-only mode. So the interpreter
//was incorrectly returning concrete 0xDE and 0xAD values in the send buffer in model_mavlink_write_tase.
//Should we just special-case the if test below for interp_only mode as a fix?
//if (isByteConcrete(offset)) {

ref<Expr> ObjectState::read8(unsigned offset) const {
  /*
  if (     (execMode == INTERP_ONLY &&  (isByteConcrete(offset)))
       ||
           (execMode == MIXED       &&  (offset< rsp_global || isByteConcrete(offset) ))
	   ) {
  */
    if (isByteConcrete(offset)) {
  //if ( offset< rsp_global || isByteConcrete(offset)) {
...
ref<Expr> ObjectState::read8(ref<Expr> offset) const {
  ...
  if (size>4096) {
    ...
    printf("WARNING: Large read flush detected \n");fflush(stdout);
