...
class CanonicalVisitor : public ExprVisitor {
 ...
 public:
  ...
  Action visitRead(const ReadExpr &e) {
    if (inModelPCS) {
      LOG_TASE("In CanonicalSolver visitRead \n");LOG_FLUSH();
    }
...
class CanonicalSolver : public klee::SolverImpl {
 ...
 public:
  ...
  bool computeValidity(const klee::Query& query, 
    ...
    printf("CALLING computeValidity in CanonicalSolver \n");fflush(stdout);
    ...
    printf("Spent %lf seconds in computeValidity in CanonicalSolver before impl call \n", util::getWallTime() - T0 );
    
    bool res = solver->impl->computeValidity(canonicalQuery, isValid);
    printf("Spent %lf seconds total in computeValidity in CanonicalSolver including impl call \n", util::getWallTime() - T0 );
    return res;
  }
  ...
  bool computeValue(const klee::Query& query, 
    ...
    LOG_TASE("In CanonicalSolver computeValue \n");LOG_FLUSH();
    klee::ref<klee::Expr> expr = query.expr;
    klee::ConstraintManager constraints(query.constraints);
    canonicalize(expr, constraints);
    klee::Query canonicalQuery(constraints, expr);
  ...
  bool computeInitialValues(const klee::Query& query,
                            ...
                            bool &hasSolution) {
    LOG_TASE("Canon computeInitialValues  \n");LOG_FLUSH();
  ...
  void publicCanonicalize(klee::ref<klee::Expr> &expr,
			  klee::ConstraintManager &constraints) {
    return canonicalize(expr, constraints);
  }
...
Canonicalizer * dummyCanonicalizer;

void klee::canonicalize(klee::ref<klee::Expr> &expr,
		   klee::ConstraintManager &constraints) {
  /*
  if (dummyCanonicalizer == NULL) {
    dummyCanonicalizer = new Canonicalizer();
  }
  dummyCanonicalizer->canonicalize(expr, constraints);
  */
  Canonicalizer dummyCanonicalizer;
  dummyCanonicalizer.canonicalize(expr,constraints);
