...
public:
  ...
  bool computeValue(const Query& query, ref<Expr> &result) {
    LOG_TASE("In cachingSolver computeValue \n");LOG_FLUSH();
  ...
  bool computeInitialValues(const Query& query,
                            ...
                            bool &hasSolution) {
    LOG_TASE("Caching computeInitialValues  \n");LOG_FLUSH();
    ++stats::queryCacheMisses;
...
bool CachingSolver::computeValidity(const Query& query,
  ...
  printf("CALLING COMPUTE VALIDITY IN CACHING SOLVER \n");fflush(stdout);
  double preCachedLookupTime = util::getWallTime();
  IncompleteSolver::PartialValidity cachedResult;
  bool tmp, cacheHit = cacheLookup(query, cachedResult);
  printf("%lf seconds elapsed doing cacheLookup in cachingSolver cacheLookup \n");fflush(stdout);
  ...
  if (!solver->impl->computeValidity(query, result)) {
    printf("%lf seconds elapsed in caching solver call to impl below \n", util::getWallTime() - T1);fflush(stdout);
    return false;
  ...
  printf("%lf seconds elapsed in caching solver in call to cacheInsert \n", util::getWallTime() - T2);fflush(stdout);
  return true;
}

bool CachingSolver::computeTruth(const Query& query,
                                 bool &isValid) {
  printf("CALLING COMPUTE TRUTH IN CACHING SOLVER \n");fflush(stdout);
  IncompleteSolver::PartialValidity cachedResult;
