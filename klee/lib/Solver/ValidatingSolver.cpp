...
bool ValidatingSolver::computeValidity(const Query &query,
  ...
  printf("ENTERING COMPUTE VALIDITY IN VALIDATING SOLVER \n");fflush(stdout);
  if (!solver->impl->computeValidity(query, result)) {
    printf("Spent %lf seconds total in computeValidity in ValidatingSolver case 1 \n", util::getWallTime() -T0);fflush(stdout);
    return false;
  }
  printf("TMP time 1 %lf \n", util::getWallTime() - T0);fflush(stdout);
  if (!oracle->impl->computeValidity(query, answer)) {
    printf("Spent %lf seconds total in computeValidity in ValidatingSolver case 2 \n", util::getWallTime() -T0);fflush(stdout);
    return false;
  }
  printf("TMP time 2 %lf \n", util::getWallTime() - T0);fflush(stdout);
  if (result != answer)
    assert(0 && "invalid solver result (computeValidity)");

  printf("Spent %lf seconds total in computeValidity in ValidatingSolver case 3 \n", util::getWallTime() -T0);fflush(stdout);
  return true;
}

bool ValidatingSolver::computeValue(const Query &query, ref<Expr> &result) {
  bool answer;
  LOG_TASE("In ValidatingSolver computeValue \n");LOG_FLUSH();
...
bool ValidatingSolver::computeInitialValues(
  ...
  LOG_TASE("VS computeInitialValues  \n");LOG_FLUSH();
  bool answer;
