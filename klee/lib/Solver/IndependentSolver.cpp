...
IndependentElementSet getIndependentConstraints(const Query& query,
  ...
  //printf("CALLING getIndependentConstraints \n");fflush(stdout);
  ...
  //printf("Spent %lf seconds in getIndependentConstraints \n", util::getWallTime() - T0);fflush(stdout);
  return eltsClosure;
}
...
bool IndependentSolver::computeValidity(const Query& query,
  ...
  printf("ENTERING COMPUTE VALIDITY IN INDEPENDENT SOLVER \n");fflush(stdout);
  ...
  if (useUF) {
    ...
    printf("arrs.size() is %d \n", arrs.size());fflush(stdout);
    for (auto it = arrs.begin(); it != arrs.end(); it++) {
      printf("Looping \n");fflush(stdout);

      int ufe = (*it)->UFE;
      printf("ufe is %d \n", ufe);fflush(stdout);
      UFElement * rep = &((*ufmanager)[ufe]);//Deref to get overloaded [] operator
      printf("rep->constraints.size() is %d \n", rep->constraints.size());
      required.insert(required.end(), rep->constraints.begin(), rep->constraints.end());
      /*
auto &rep = (*ufmanager)[ufmanager->find(ufe)];
      printf("rep.constraints.size() is %d \n", rep.constraints.size());fflush(stdout);
      required.insert(required.end(), rep.constraints.begin(), rep.constraints.end());      
      */
    }
    ...
    T0 = util::getWallTime();
    if (taseDebug) {
      outs() << "Printing required constraints in independent solver \n";
      for (int i = 0; i < required.size(); i++) {
	outs().flush();
	required[i]->dumpToStdout();
	outs().flush();
      }
    }
  ...
  //printf("Required is size %d \n", required.size());fflush(stdout);
  double preSolverTime = util::getWallTime();
  bool res = solver->impl->computeValidity(Query(tmp, query.expr),
					   result);
  //printf("computeValidity core solver time %lf \n", util::getWallTime() - preSolverTime);fflush(stdout);
  //printf("TOTAL TIME IN COMPUTE VALIDITY FOR INDEPENENT SOLVER: %lf \n", util::getWallTime() - entryTime);
  //fflush(stdout);
  return res;
...
bool IndependentSolver::computeTruth(const Query& query, bool &isValid) {
  //printf("CALLING COMPUTE TRUTH IN INDEPENDENT SOLVER \n");fflush(stdout);
  std::vector< ref<Expr> > required;
  IndependentElementSet eltsClosure = 
    getIndependentConstraints(query, required);
  ConstraintManager tmp(required);
...
bool IndependentSolver::computeValue(const Query& query, ref<Expr> &result) {
  LOG_TASE("In Independent Solver compute value \n");LOG_FLUSH();
  std::vector< ref<Expr> > required;
  IndependentElementSet eltsClosure = 
    getIndependentConstraints(query, required);
  ConstraintManager tmp(required);
