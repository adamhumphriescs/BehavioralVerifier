...
protected:
  ref<Expr> getInitialValue(const Array& array, unsigned index) {
    ...
    LOG_TASE("In CexPossibleEvaluator getInitialValue \n");LOG_FLUSH();
...
protected:
  ref<Expr> getInitialValue(const Array& array, unsigned index) {
    // If the index is out of range, we cannot assign it a value, since that
    // value cannot be part of the assignment.
    LOG_TASE("In CexExactEvaluator getInitialValue \n");LOG_FLUSH();
...
FastCexSolver::computeTruth(const Query& query) {
  printf("CALLING COMPUTE TRUTH IN FAST CEX SOLVER \n");fflush(stdout);
  CexData cd;
...
bool FastCexSolver::computeValue(const Query& query, ref<Expr> &result) {
  LOG_TASE("In FastCex computeValue \n");LOG_FLUSH();
  CexData cd;
...
FastCexSolver::computeInitialValues(const Query& query,
                                    ...
                                    std::vector< std::vector<unsigned char> >
                                      &values,
                                    bool &hasSolution) {
  LOG_TASE("FCS computeInitialValues  \n");LOG_FLUSH();
  CexData cd;
