...
bool CexCachingSolver::getAssignment(const Query& query, Assignment *&result) {
  //LOG_TASE("In CexCachingSolver getAssignment \n");LOG_FLUSH();
  KeyType key;
...
bool CexCachingSolver::computeValidity(const Query& query,
  ...
  //printf("CALLING COMPUTE VALIDITY IN CEX CACHING SOLVER \n");fflush(stdout);
...
bool CexCachingSolver::computeTruth(const Query& query,
                                    bool &isValid) {
  TimerStatIncrementer t(stats::cexCacheTime);
  //printf("CALLING COMPUTE TRUTH IN CEX CACHING SOLVER \n");fflush(stdout);
...
bool CexCachingSolver::computeValue(const Query& query,
                                    ref<Expr> &result) {
  //LOG_TASE("In CexCachingSolver computeValue \n");LOG_FLUSH();
  TimerStatIncrementer t(stats::cexCacheTime);
  //LOG_TASE("DBG 1a \n");LOG_FLUSH();
  Assignment *a;
  if (!getAssignment(query.withFalse(), a))
    return false;
  //LOG_TASE("DBG 2a \n");LOG_FLUSH();
  //LOG_TASE("Assignment a at 0x%lx \n", (uint64_t) a);LOG_FLUSH();
...
CexCachingSolver::computeInitialValues(const Query& query,
                                       ...
                                       std::vector< std::vector<unsigned char> >
                                         &values,
                                       bool &hasSolution) {
  //LOG_TASE("In CexCache ComputeInitialValues \n");LOG_FLUSH();
  TimerStatIncrementer t(stats::cexCacheTime);
  Assignment *a;
