...
public:
  ...
  //Add extra "cheat" constraints for use in steering path selection when training a path selection algorithm.
  ...
  //ABH added
  std::vector< ref<Expr>> getConstraintVector() {
    return constraints;
  }
...
private:
  ...
  std::vector< ref<Expr> > cheatConstraints;
  ...
  void addConstraintInternal(ref<Expr> e, std::vector<const klee::Array * > * symNames = NULL, bool cheat = false);
  //void addConstraintInternal(ref<Expr> e, const std::vector<const klee::Array * > * symNames = NULL, bool cheat = false);
